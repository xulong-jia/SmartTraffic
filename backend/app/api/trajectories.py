...
from app.repositories import TrajectoryPointRepository, TrafficAnalysisRunRepository
...
def list_trajectories(
    ...
    rows = TrajectoryPointRepository(db).list(run_id=run_id, video_id=video_id)
    ...
    if rows or run_id is None or TrafficAnalysisRunRepository(db).get(run_id) is not None:
        ...
        return {
            ...
            "summary": {"total_trajectory_points": len(rows)},
            "frames": _group_by_frame(limited),
            "rows": [_trajectory_payload(row) for row in limited],
            "limit": limit,
            "track_id": track_id,
            "source": "db",
        }
    try:
        ...
        raise HTTPException(
            ...
            detail="trajectory points not found",
        ) from exc


def _trajectory_payload(row: Any) -> dict[str, Any]:
    return {
        "id": row.id,
        "run_id": row.run_id,
...
def _group_by_frame(rows: list[Any]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        frame["trajectory_points"].append(_trajectory_payload(row))
