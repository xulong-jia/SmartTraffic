...
本文档记录 Stage 9CD demo / sample 收口结果。Stage 9CD 只提供小型配置和 toy expected annotation，不提交视频、模型权重或 generated results。
...
- `scripts/seed_demo_data.py` 是 Stage 9CD demo seed MVP，可生成小型 demo/sample 配置和 toy expected annotation。
...
Stage 9CD 已补充以下小型文本配置：

- `samples/configs/demo_zones.json`：1 个 vehicle lane、1 个 danger zone、1 个 counting line。
- `samples/configs/demo_event_rules.json`：wrong-way、danger-zone intrusion、flow-counting 三条 toy event rules。
- `samples/configs/demo_processing_request.json`：可用于 `POST /api/videos/{video_id}/process` 的 dry-run request body 示例。
- `evals/expected/demo_expected_events.json`：3 条 toy expected event。
