...
foundation, and Stage 2CD DB-backed core result persistence.

本文描述当前 schema/repository foundation、Full Stage 2 DB-backed 核心流范围和目标数据库边界，不代表当前仓库已经完成 DB-backed 全量业务迁移。
...
- Videos, processing tasks, processing-created run indexes, detections, tracks,
  trajectory points, flow counts, and zone statistics are DB-backed.
...
tables. Current Event / Alert lifecycle, Review, Bad Case, and Evaluation
workflow endpoints still use the artifact-backed MVP path; this is not the
DB-backed full final version.
